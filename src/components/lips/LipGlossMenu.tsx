...
interface LipGlossMenuProps {
  titlebrand: string;
  onBack: () => void;
}

export function LipGlossMenu({ titlebrand, onBack }: LipGlossMenuProps) {
  ...
  if (currentView === 'colorFamily') {
    return (
      <LipsColorPicker 
        title={"LipGloss Colors"}
        colors={LIP_COLORS}
        onBackToMenu={handleBack}
        onColorSelect={(color) => {
          setSelectedColor(color);
        }}
        minId={16} // Pass minId for filtering
        maxId={30} // Pass maxId for filtering
      />
    );
  }
