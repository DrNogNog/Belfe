...
interface LipLinerMenuProps {
  titlebrand: string;
  onBack: () => void;
}

export function LipLinerMenu({ titlebrand, onBack }: LipLinerMenuProps) {
  ...
  if (currentView === 'colorFamily') {
    return (
      <LipsColorPicker 
        title={"LipLiner Colors"}
        colors={LIP_COLORS}
        onBackToMenu={handleBack}
        onColorSelect={(color) => {
          setSelectedColor(color);
        }}
        minId={46} // Pass minId for filtering
        maxId={60} // Pass maxId for filtering
      />
    );
  }
