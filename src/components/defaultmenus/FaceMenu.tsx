...
import { FoundationMenu } from '../face/FoundationMenu';
import { ConcealerMenu } from '../face/ConcealerMenu';
import { SettingPowderMenu } from '../face/SettingPowderMenu';

interface FaceMenuProps {
  onBack: () => void;
  onEyesClick: () => void;
  onLipClick: () => void;
  onCheeksClick: () => void;
...
export function FaceMenu({ 
  ...
  const renderSubMenu = () => {
    switch (currentSubMenu) {
      case 'foundation':
        return <FoundationMenu onBack={() => setCurrentSubMenu(null)} />;
      case 'concealer':
        return <ConcealerMenu onBack={() => setCurrentSubMenu(null)} />;
      case 'settingpowder':
        return <SettingPowderMenu onBack={() => setCurrentSubMenu(null)} />;
      default:
        return null;
    }
  };
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="pl-12 space-y-2">
              <button
                onClick={() => setCurrentSubMenu('foundation')}
                className="w-full text-left text-lg hover:text-white/80 transition-colors"
              >
                Foundation
              </button>
              <button
                onClick={() => setCurrentSubMenu('concealer')}
                className="w-full text-left text-lg hover:text-white/80 transition-colors"
              >
                Concealer
              </button>
              <button
                onClick={() => setCurrentSubMenu('settingpowder')}
                className="w-full text-left text-lg hover:text-white/80 transition-colors"
              >
                Setting Powders
              </button>
